...
export function filterRows(rows, filters) {
  ...
  return rows.filter((row) => {
    return Object.keys(filters).every((accessor) => {
      ...
      // if the search column is object it MUST has 'name' properity to search
...
export function sortRows(rows, sort) {
  return rows.sort((a, b) => {
    ...
    // if the sort column is object it MUST has 'name' properity to sort
    if (typeof aLocale === "object") {
      aLocale = convertType(a[orderBy].name);
      bLocale = convertType(b[orderBy].name);
    }
